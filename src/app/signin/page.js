     ...
     e.preventDefault()
        if( !userData.username && !userData.email && !userData.password && !userData.repeatPassword && !userData.terms )      return toast.warn("all fields are required !")
          ...
          try {
            ...
            if(!response.ok) toast.error(result.error.message || "Inernal error")
  ...
  return (
    ...
    <form className="max-w-sm mx-auto" onSubmit={handleSubmit}>
      ...
      <div className="flex items-start mb-5 ml-4">
        <p  className="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Already have an account <Link href="/signup" className="text-blue-600 hover:underline dark:text-blue-500">Signup</Link></p>
      </div>
